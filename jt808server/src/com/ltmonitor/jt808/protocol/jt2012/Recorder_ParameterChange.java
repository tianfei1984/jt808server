	...
	public final void ReadFromBytes(byte[] bytes) {
		...
		if (bytes != null) {
			for (int i = 0; i < bytes.length / 7; i++) {
				...
				String strTime = new java.util.Date(java.util.Date.parse("20"
						+ String.format("%02X", EventTime[0]) + "-"
						+ String.format("%02X", EventTime[1]) + "-"
						+ String.format("%02X", EventTime[2]) + " "
						+ String.format("%02X", EventTime[3]) + ":"
						+ String.format("%02X", EventTime[4]) + ":"
						+ String.format("%02X", EventTime[5]))).toString();
				vr.setStartTime(DateUtil.stringToDateTime(strTime));
				String eventType = "";
				switch (bytes[6 + 7 * i]) {
				case 0x00:
